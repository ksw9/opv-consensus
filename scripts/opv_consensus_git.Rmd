...
knitr::opts_knit$set(root.dir = '~/Desktop/For Utah Box/Polio/opv-consensus/')
library(tidyverse)
library(ggsci)
library(ggthemes)
library(cowplot)
...
project_dir = '~/Desktop/For Utah Box/Polio/opv-consensus/'
```
```{r fig1}
...
#### Try this: read in genbank alignments
# Read alignments to full reference sequence. (Needs to be DNAbin, matrix which signifies aligned sequences)
vp1_1 <- read.dna(file = paste0(project_dir,'processed/aln/opv1_vp1_ref.fasta'),format = 'fasta')
vp1_2 <- read.dna(file = paste0(project_dir,'processed/aln/opv2_vp1_ref.fasta'), format = 'fasta')
vp1_3 <- read.dna(file =  paste0(project_dir,'processed/aln/opv3_vp1_ref.fasta'), format = 'fasta')
...
# Read in metadata
meta <- read_csv(paste0(meta_dir,'metadata_opv.csv'))
```
...
d_pairs <- d_long %>% bind_rows(d_neutral_long) %>% 
  ...
  mutate(loc.x = str_sub(p.x, start = 1, end = 1),
         ...
         pair_type = case_when(str_starts(p.x, 'AY1') | str_starts(p.y, 'AY1')  ~ 'ref',
                               ind.x == ind.y ~ 'host',
...
ggsave(p_all, filename = paste0(plot_dir,'haplotypes.pdf'))
```
...
# Translate OPV-1 slope to substitution rate
0.11* 1/906
...
table(as.character(vp1_1[,opv1_att1-opv1_start + 1])) 
rev1a <- labels(vp1_1[which(as.character(vp1_1[,opv1_att1-opv1_start + 1]) != 'a'),])
length(rev1a)/49
d_pairs %>% ungroup() %>% 
  filter(str_starts(p.x,'AY1') & aln_type == 'all') %>% 
  filter(p.y %in% rev1a) %>% relocate(VacStatus.y, daysPost.y)

# Look at OPV1 opv1_att2 in full length alignment
table(as.character(aln1[,opv1_att2-opv1_start + 1])) 
rev1b <- labels(vp1_1[which(as.character(vp1_1[,opv1_att2-opv1_start + 1]) != 'a'),])
length(rev1b)/49
d_pairs %>% ungroup() %>% 
  filter(str_starts(p.x,'AY1') & aln_type == 'all' & serotype == 1) %>% 
  filter(p.y %in% rev1b) %>% relocate(VacStatus.y, daysPost.y)
table(as.character(vp1_1[,opv1_att3-opv1_start + 1])) # no mutations

# OPV2
table(as.character(vp1_2[,opv2_att1-opv2_start + 1]))
rev2 <- labels(vp1_2[which(as.character(vp1_2[,opv2_att1-opv2_start + 1]) != 't'),])
length(rev2)/136
d_pairs %>% ungroup() %>% 
  filter(str_starts(p.x,'AY1') & aln_type == 'all' & serotype == 2) %>% 
  filter(p.y %in% rev2) %>% relocate(VacStatus.y, daysPost.y,ind.y) %>% arrange(ind.y)

table(as.character(vp1_2[,opv2_att1-opv2_start])) # no evidence of mutation at the adjacent site. 

# OPV3 in full length alignment
table(as.character(vp1_3[,opv3_att1-opv3_start + 1]))
mixed <- labels(vp1_3[which(as.character(vp1_3[,opv3_att1-opv3_start + 1]) %in% c('n','-')),])
as.character(vp1_3[mixed,opv3_att1-opv3_start + 1])
rev3 <- labels(vp1_3[which(!as.character(vp1_3[,opv3_att1-opv3_start + 1]) %in% c('c','n','-')),])
length(rev3)/173
d_pairs %>% ungroup() %>% 
...
d_pairs %>% ungroup() %>% 
  filter(str_starts(p.x,'AY1') & aln_type == 'all' & serotype == 3) %>% 
  filter(p.y %in% tmp) %>% relocate(VacStatus.y, daysPost.y,ind.y) %>% arrange(-daysPost.y)
...
# OPV1
opv1_summary <- as.data.frame(as.character(vp1_1[,c(opv1_att1-opv1_start + 1,opv1_att2-opv1_start + 1,opv1_att3-opv1_start + 1)])) %>% 
...
# OPV2
opv2_summary <- as.data.frame(as.character(aln2[,c(opv2_att1-opv2_start + 1)])) %>% 
  rownames_to_column(var = 'Sample') %>% as_tibble() %>% 
...
#OPV3
opv3_summary <- as.data.frame(as.character(aln3[,c(opv3_att1-opv3_start + 1)])) %>% 
  rownames_to_column(var = 'Sample') %>% as_tibble() %>% 
